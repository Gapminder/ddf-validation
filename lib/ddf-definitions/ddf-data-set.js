...
class DdfDataSet {
  ...
  load(cb) {
    ...
    this.ddfRoot.check(() => {
      ...
      this.nonIndexFileDescriptors = _.reduce(
        this.ddfRoot.directoryDescriptors.filter(directoryDescriptor => directoryDescriptor.isDDF),
        (aggregation, directoryDescriptor) => aggregation.concat(
          _.differenceWith(
            directoryDescriptor.fileDescriptors,
            directoryDescriptor.ddfIndex.fileDescriptors,
            (fileDescriptor, fileDescriptorFromIndex) => fileDescriptor.file === fileDescriptorFromIndex.file
          )
        ),
        []
      );
