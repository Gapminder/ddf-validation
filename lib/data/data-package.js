...
const Concept = require('../ddf-definitions/concept');
  ...
  fillPrimaryKeys(conceptTypeHash) {
    ...
    const fillEntityPrimaryKey = fileDescriptor => {
      ...
      if (fileDescriptor.parts.length !== ONLY_TWO) {
        fileDescriptor.primaryKey = _.first(fileDescriptor.parts);
        return;
      }
    ...
    const fillDataPointPrimaryKey = fileDescriptor => {
      const ddfDataPointSeparatorPos = _.indexOf(fileDescriptor.parts, constants.DDF_DATAPOINT_SEPARATOR);
      const primaryKeyPartsCount = fileDescriptor.parts.length - ddfDataPointSeparatorPos - 1;

      fileDescriptor.primaryKey = _.takeRight(fileDescriptor.parts, primaryKeyPartsCount);
    };
        ...
        (translationsErr, translationFolders) => {
          ...
          this.fillHeaders(headersError => {
            ...
            const conceptLoadingActions = this.fileDescriptors
              ...
              .map(fileDescriptor => {
                return onFileLoaded => {
                  this.db.fillCollection(Symbol.keyFor(fileDescriptor.type), fileDescriptor.fullPath, onFileLoaded);
                };
              });

            async.parallelLimit(conceptLoadingActions, PROCESS_LIMIT, conceptsErr => {
              if (conceptsErr) {
