...
class DDFRoot {
  ...
  getChecks(dirs, excludedDirs) {
    const actions = [];

    dirs.concat(this.path).forEach(dir => {
      const isAllowed = _.isEmpty(_.compact(
        excludedDirs.map(excludedDir => isExcludedDirectory(dir, excludedDir))
  ...
  check(excludedDirs, cb) {
    fu.walkDir(this.path, (err, dirs) => {
      ...
      async.parallel(this.getChecks(dirs, excludedDirs), (_err, directoryDescriptors) => {
