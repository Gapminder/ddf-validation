...
const CASES = [
  {
    check: filename => {
      ...
      const ok = isConceptSpecific(headersFromFilename);
      const details = {concepts: ['concept', 'concept_type']};

      return {ok, details, headers: []};
    },
  ...
  },
  {
    check: filename => {
      const headersFromFilename = baseFilenameCheck(filename);
      const ok = isEntitySpecific(headersFromFilename);
      const details =
      {concepts: ok ? headersFromFilename.spl.slice(START_INDEX, headersFromFilename.spl.length - 1) : []};

      return {ok, details, headers: headersFromFilename};
    },
  ...
  },
  {
    check: filename => {
      const headersFromFilename = baseFilenameCheck(filename);
      const ok = isDataPointSpecific(headersFromFilename);
      const allConcepts = ok ? headersFromFilename.spl.slice(START_INDEX, headersFromFilename.spl.length - 1) : [];
...
class DirectoryDescriptor {
  ...
  getFileDescriptor(dir, file) {
    ...
    for (const currentCase of CASES) {
      const checkResultTemp = currentCase.check(file);

      if (checkResultTemp.ok) {
        checkResult = checkResultTemp;
