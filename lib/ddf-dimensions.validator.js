  ...
  return isHeaderValid$.combineLatest([isConceptIdUnique$, isRowsValid$.toArray()],
    (isHeaderValid, isConceptIdUnique, isRowsValid)=> {
      ...
      if (!isHeaderValid.isValid) {
        logger.notice('header is not valid', isHeaderValid.errors[0].message);
        isHeaderValid.errors = JSON.stringify(isHeaderValid.errors);
        errors.push(isHeaderValid);
      }
      if (!isConceptIdUnique.isValid) {
        logger.notice('concept id is not unique', isConceptIdUnique.errors[0].message);
      ...
      isRowsValid.forEach(isRowValid=>{
        if (!isRowValid.isValid) {
          logger.notice('entry is not valid', isRowValid.errors[0].message);
      ...
      if (errors.length === 0) {
        logger.notice(`${fileName} in ${folderPath} is valid`);
        return [{
          fileName: fileName,
          folder: folderPath,
          valid: true
