...
function getWrongIndexKeyOrValueIssues(kind, rule) {
  return (ddfDataSet, ddfIndex) => {
    function getUndefinedConcept(concept) {
      ...
      if (!!foundConcept || _.includes(CONCEPT_TERMS, concept)) {
        return null;
      }
    ...
    const wrongKeys = _.compact(_.flatten(
      ddfIndex.content.map(indexRecord => {
        const keys = _.split(indexRecord[kind], ',').map(key => key.replace(/^is--/, ''));
...
module.exports = {
  ...
  [registry.INDEX_IS_NOT_FOUND]: ddfDataSet => ddfDataSet.ddfRoot.directoryDescriptors
    .filter(() => !ddfDataSet.ddfRoot.settings.indexlessMode)
    .filter(directoryDescriptor => directoryDescriptor.isDDF && !directoryDescriptor.ddfIndex.exists)
