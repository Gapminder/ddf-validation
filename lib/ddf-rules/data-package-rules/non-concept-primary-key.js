  ...
  rule: ddfDataSet => _.compact(_.flattenDeep(
    ddfDataSet.ddfRoot.directoryDescriptors.map(directoryDescriptor =>
      directoryDescriptor.dataPackage.getResources().map(resource => {
        ...
        const allConceptsIds = ddfDataSet.getConcept().getIds().concat(constants.PREDEFINED_CONCEPTS);
        const nonConcepts = dataPackagePrimaryKey.filter(header => !_.includes(allConceptsIds, header));
