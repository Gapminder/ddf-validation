...
module.exports = {
  [registry.WRONG_ENTITY_IS_HEADER]: ddfDataSet => {
    ...
    function getNonConceptData(actualHeaderDetail, headerDetail) {
      return conceptIds.indexOf(actualHeaderDetail) < 0 ? {
        message: 'Not a concept',
        headerDetail
      } : null;
    }

    function getWrongTypeData(actualHeaderDetail, headerDetail) {
    ...
    ddfDataSet.getEntity().details.forEach(detail => {
      detail.header.forEach(headerDetail => {
        ...
        if (_.startsWith(headerDetail, IS_HEADER_PREFIX)) {
          ...
          const data = getNonConceptData(actualHeaderDetail, headerDetail) ||
            getWrongTypeData(actualHeaderDetail, headerDetail);
