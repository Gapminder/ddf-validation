...
function constructEntityCondition(entity) {
  ...
  return {[expectedKey]: {$in: ['1', 'TRUE', 'true']}};
}
...
const dataFor = {
  ...
  entityValueHash: dataPointDescriptor => {
    ...
    if (!cache[key]) {
      const conceptTypeDictionary = dataFor.conceptTypeDictionary(dataPointDescriptor);
      ...
      const conceptDomainDictionary = dataFor.conceptDomainDictionary(dataPointDescriptor);
  ...
  timeKeys: dataPointDescriptor => {
    const key = `timeKeys@${dataPointDescriptor.dataPointDetail.fileDescriptor.file}`;

    if (!cache[key]) {
      const conceptTypeDictionary = dataFor.conceptTypeDictionary(dataPointDescriptor);

      cache[key] = Object.keys(conceptTypeDictionary)
        .filter(conceptTypeKey => conceptTypeDictionary[conceptTypeKey] === 'time');
    }
...
module.exports = {
  [registry.MEASURE_VALUE_NOT_NUMERIC]: dataPointDescriptor =>
    dataPointDescriptor.dataPointDetail.fileDescriptor.details.measures
      .filter(measure => isNaN(dataPointDescriptor.dataPointRecord[measure]))
  ...
  [registry.DATA_POINT_UNEXPECTED_ENTITY_VALUE]: dataPointDescriptor => {
    const entityValueHash = dataFor.entityValueHash(dataPointDescriptor);
  ...
  [registry.DATA_POINT_UNEXPECTED_TIME_VALUE]: dataPointDescriptor =>
    dataFor.timeKeys(dataPointDescriptor)
