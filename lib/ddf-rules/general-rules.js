...
function getIssueByConceptsFromFileName(conceptsStringFromFileName, fileDescriptor, prefix) {
  ...
  function includeHeaderIs(conceptFromFileName) {
    return _.includes(fileDescriptor.headers, `${prefix}${conceptFromFileName}`);
  }
...
const headerValidator = {
  ...
  [constants.ENTITY]: (ddfDataSet, fileDescriptor) => {
    ...
    return getIssueByConceptsFromFileName(conceptsStringFromFileName, fileDescriptor);
  },
  [constants.DATA_POINT]: (ddfDataSet, fileDescriptor) => {
    ...
    return getIssueByConceptsFromFileName(conceptsStringFromFileName, fileDescriptor, 'is--');
  }
};
...
function getIncorrectConceptIdentifierIssues(ddfDataSet) {
  return ddfDataSet.getConcept()
    ...
    .filter(conceptRecord => /[^a-z0-9_]/.test(conceptRecord.concept))
    .map(conceptRecord =>
